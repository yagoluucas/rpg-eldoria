...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function escolhaArmaAliado(event) {
        ...
        setTimeout(() => {
            ...
            geral.escolhasPersonagem[0].innerHTML = '<img class="imagem-arma" src="./image/arma-1.png" alt="Machado da Tempestade"><br><span class="arma">Machado da Tempestade</span><span> feito com aço temperado. Pesado e intimidador</span>'
            geral.escolhasPersonagem[1].innerHTML = '<img class="imagem-arma" src="./image/arma-2.png" alt="Espada da Aurora"><br><span class="arma">Espada da Aurora</span><span> forjada com maestria por hábeis artesãos. Brilha com a luz do amanhecer, refletindo esperança e beleza</span>'
            geral.escolhasPersonagem[2].innerHTML = '<img class="imagem-arma" src="./image/arma-3.png" alt="Cajado arcano"><br><span class="arma">Cajado arcano</span> <span> adornado com entalhes místicos que capturam a essência do desconhecido</span>'
        }, 7000)
    ...
    function escolherArma(e) {
        ...
        geral.escolhasPersonagem[0].parentElement.classList.add('none')
        let vogalArma = e.currentTarget.firstElementChild.getAttribute('alt') == 'Espada da Aurora' ? 'a ' : 'o '
        geral.revelarDialogo(0, geral.dialogo[1], `<span class="aliado-nome">${localStorage.nomeAliado}</span>: Com ${vogalArma.concat(e.currentTarget.firstElementChild.getAttribute('alt'))} você terá grandes chances de derrotar Malvagor`)
        ...
        localStorage.setItem('armaEscolhida', e.currentTarget.firstElementChild.getAttribute('alt'))
    ...
    function finalizarCapitulo() {
        localStorage.nomeAliado == 'Aldrin' ? geral.somFloresta.pause() : geral.somMontanha.pause()
        document.body.className = 'carregar-conteudo'
        clearInterval(somAmbiente)
        geral.anima('animate__fadeIn', tituloTrocaDeCenario)
    ...
    // função que remove o cenario após o usuario fazer a escolha
